...
class EmployeeMonthlyScore(object):
    ...
    def update_monthly_score_and_num_inspections(cls, emp_id, month, year, score, num_inspections):
        month_score = EmployeeMonthlyScore.get_by_emp_id_month_year(emp_id, month, year)
        if month_score is not None and month_score['num_inspections'] == 1 and num_inspections == -1:
            EmployeeMonthlyScore.remove_by_emp_id_and_month_and_year(emp_id, month, year)
        else:
            Database.update(collection, {"emp_id": emp_id, "month": month, "year": year},
